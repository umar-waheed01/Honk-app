...
export default function MemoryDetails({ route }) {
  ...
  const fetchMemory = async () => {
    try {
      const result = await apiCall(`api/memories/${friendId}/${memoryId}`, "GET");
  ...
  const handleLike = async () => {
    ...
    try {
      const result = await apiCall(
        `api/memories/${friendId}/${memoryId}/message`,
        "POST",
        requestBody
      );
  ...
  const handleSendMessage = async () => {
    if (newMessage.trim() !== "") {
      if (editingIndex !== null) {
        ...
        try {
          const result = await apiCall(
            `memories/me/${memoryId}/message/${updatedMessages[editingIndex].id}`,
            "PATCH",
            requestBody
          );
        ...
        try {
          const result = await apiCall(
            `memories/${friendId}/${memoryId}/message`,
            "POST",
            requestBody
          );
          const data = await result.json();
  ...
  const handleClearMessage = async (id) => {
    const result = await apiCall(`api/memories/${friendId}/${memoryId}/message/${id}`, "DELETE");
                        ...
                        <View style={styles.messageContainer}>
                          ...
                          {element.authorId === user?.id ? (
                            <View style={styles.iconsContainer}>
                              <TouchableOpacity onPress={() => handleEditMessage(index)}>
